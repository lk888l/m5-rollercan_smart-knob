...
bool writeMessageToFlash( uint8_t *buff , uint16_t length)
{
    ...
    /*Write head*/
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE59_STARTADDR, temp) != HAL_OK) {
        HAL_FLASH_Lock();
        return false;
    }
    /*Write length*/
    ...
    for(i=0 ;i<length/8 ;i++)
    {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, STM32G0xx_FLASH_PAGE59_STARTADDR+8+8*i, temp) != HAL_OK) {
            HAL_FLASH_Lock();
            return false;
        }
    }  
