...
This is a locally autonomous SmartKnob firmware for the M5 Stack RollerCAN, derived from M5Stack's [official open-source firmware](https://github.com/m5stack/M5Unit-RollerCAN-Internal-FW). FOC, haptics, mode selection, parameter editing, and the OLED UI all run on the STM32G431; the existing RollerCAN CAN-FD host protocol can take temporary priority without replacing the local UI.

The firmware initializes FDCAN and CommunicationTask in fixed CAN-FD+BRS mode (1 Mbit/s nominal, 5 Mbit/s data). Read-only discovery does not interfere with local operation. The first supported write addressed to this node gives the host control until three seconds of valid-traffic silence.
...
- Running 12 local Dial/SmartKnob haptic presets.
- Operating the local menu through detents, long presses, and double-clicks.
- Displaying a round speed gauge, detent position, mode, and faults on the 64×48 SSD1306 OLED.
- Showing run, pause, menu, save, and fault states with deterministic RGB effects.
- Saving all 12 per-mode haptic tunings, Custom configuration, local fields, CAN ID, and encoder offset to internal Flash.
...
| [Firmware SmartKnob](docs/smartknob-firmware.md) | Local presets, runtime profiles, navigation detents, current limiting, and CAN host control |
| [Communication Protocol](docs/communication-protocol.md) | I2C register table, CAN commands, and the CAN-I2C bridge |
| [Display and Input](docs/display-and-input.md) | Round gauge, local menu, long press/double-click behavior, parameter ranges, and safety boundaries |
...
After `main()` completes peripheral initialization and `InitMysys()`, it starts FreeRTOS. The 1 kHz ControlTask owns protection, SmartKnob, local state, and host commands. MaintenanceTask handles the button, OLED, and LEDs; CommunicationTask drains CAN RX/TX; StorageTask writes snapshots only after torque output is disabled. At approximately 18.67 kHz, the TIM1 interrupt starts an encoder DMA transfer and the DMA2 RX completion continues with FOC.
...
Power-on
  ...
  -> InitMysys
       -> ADC DMA / TIM1 PWM / motor driver / encoder / local Flash profile / OLED
       -> Rebase multi-turn tracking after loading the encoder offset
       -> Start local Dial output; enable the fixed-timing CAN-FD interface
  -> App_StartScheduler
       -> ControlTask: 1 kHz outer loops, protection, SmartKnob, and local command execution
       -> MaintenanceTask: button events, gauge, local menu, and lighting
       -> CommunicationTask: CAN-FD RX/TX, responses, and telemetry
...
- Double-click on the dashboard to enable or pause haptic output.
- Hold for about 1.2 seconds to enter the local menu.
- Rotate through the menu using the private 12° navigation detents.
- Double-click to enter or confirm; hold in an editor to cancel; hold at the root to save and exit.
- The menu configures `MODE`, `FORCE`, `STEP`, and `LIMIT`.

Saving first commands zero current and disables the driver. StorageTask writes and verifies Flash, then restores the run/pause state that was active before entering the menu. See [Display and Input](docs/display-and-input.md) for the complete ranges and mode abbreviations.

## CAN FD Bus Configuration

The runtime interface is always enabled and remains wire-compatible with the existing RollerCAN host. The stored node ID is preserved (default `0xA8`), while `comm_type=COMM_TYPE_CAN` and `bps_index=0` are enforced at runtime.
...
Baud-rate write commands are acknowledged for protocol compatibility but the
runtime timing remains fixed at 1/5 Mbit/s, so the host cannot accidentally
move the node away from the GUI's required bus timing.
...
## Host Compatibility and `*`

Ping and function reads are discovery traffic and never lock the local controls. The first supported local write acquires host ownership, cancels any uncommitted menu draft, and keeps the OLED live. While owned, the bottom row shows `HOST CONNECTED`; local menu entry and double-click enable/disable are ignored. Valid ping/read/write traffic refreshes ownership, and three seconds of silence releases it.

The host startup transaction is recognized as disable → zero current → select/configure mode → Dial → enable. A partial transaction is never committed. Stop keeps host ownership and the motor disabled while host traffic continues. On Stop or disconnect, a successfully enabled full haptic snapshot is saved; on disconnect, the pre-host run/pause state is restored only after a verified Flash write and only when no protection fault is active.

`HOST CONNECTED*` means the active full configuration is valid and preserved, but contains values the four-field OLED editor cannot represent exactly. An affected numeric editor displays `*`; the first turn starts from its nearest legal local value. Saving locally changes only fields actually edited, so hidden P/D, friction, click, and other advanced values remain intact and the global `*` may remain.
...
- After the scheduler starts, local UI and CAN commands must enter ControlTask through the static queue; CommunicationTask must not mutate control state directly.
