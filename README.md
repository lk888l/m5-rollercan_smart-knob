...
This is a standalone SmartKnob firmware for the M5 Stack RollerCAN, derived from M5Stack's [official open-source firmware](https://github.com/m5stack/M5Unit-RollerCAN-Internal-FW). FOC, haptics, mode selection, parameter editing, and the OLED UI all run on the STM32G431 without a CANFD host.

The legacy I2C/CANFD protocol source remains for reference, but the current entry point does not initialize FDCAN, does not create the communication task, and keeps the external transceiver in standby.
...
- Saving mode, force, detent angle, current limit, and encoder offset to internal Flash.
...
| [Firmware SmartKnob](docs/smartknob-firmware.md) | Local presets, runtime profiles, navigation detents, current limiting, and legacy CAN reference |
...
After `main()` completes peripheral initialization and `InitMysys()`, it starts FreeRTOS. The 1 kHz ControlTask owns protection, SmartKnob, and local control state. MaintenanceTask handles the button, OLED, and LEDs, while StorageTask saves local settings after torque output is disabled. CommunicationTask is not created in this build. At approximately 18.67 kHz, the TIM1 interrupt starts an encoder DMA transfer and the DMA2 RX completion continues with FOC.
...
Power-on
  ...
  -> InitMysys
       ...
       -> Start local Dial output; keep CAN transceiver in standby
  -> App_StartScheduler
...
This section documents the retained legacy protocol. The standalone entry point does not call `MX_FDCAN1_Init()` or create CommunicationTask. Restoring bus control requires re-enabling both.
...
- After the scheduler starts, local UI commands must enter ControlTask through the static queue. If legacy CAN is re-enabled, CommunicationTask must follow the same single-writer boundary.
