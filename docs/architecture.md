...
| 业务控制层 | `MyFile/src/*.c`, `MyFile/inc/*.h`, `Core/Src/flash.c`, `Core/Src/i2c_ex.c` | 电机控制、本地 UI、旧通信协议、灯效、参数保存 |
...
| CommunicationTask | `App/src/app_rtos.cpp` | 仅在 `comm_type != COMM_TYPE_NONE` 时创建；本地直运行配置不创建 |
...
调度器启动后，本地 UI 路径遵守 ControlTask 单写者模型。当前入口不初始化 FDCAN、不启动 I2C 从机业务，也不创建 CommunicationTask；旧 CAN/I2C 源码仅作为可选协议参考保留。
...
| `motor_output` | `MyFile/src/mysys.c` | 本地 UI 控制的力反馈开关 |
| `speed_point`, `pos_point`, `current_point` | `MyFile/src/mysys.c` | 旧速度/位置/电流模式目标；本地构建固定使用 Dial |
...
| `comm_type` | `MyFile/src/u8g2_disp_fun.c` | 本地构建固定为 `COMM_TYPE_NONE` |

## CubeMX 与手写代码边界

`Core/Src/*.c` 大部分是 CubeMX 生成文件，但本工程在几个文件的 USER CODE 区加入了业务逻辑：

- `main.c`：启动前将 `SCB->VTOR` 指向 `0x08002000` 的完整应用向量表、Flash 参数初始化/写回、I2C 从机协议分发；当前入口将 CAN 收发器置于 standby 且不调用 FDCAN 初始化。
